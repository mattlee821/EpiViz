...
**EpiViz** is an R package and `Shiny` application designed to streamline the creation of Circos plots specifically for epidemiologists. It acts as a high-level wrapper around the powerful `circlize` and `ComplexHeatmap` packages.
...
For researchers who prefer a graphical interface, the `Shiny` application provides a similar experience to the R package without requiring any coding.

- **Hosted Version**: [EpiViz on ShinyApps.io](https://mattlee.shinyapps.io/EpiViz/)
- **Features**: Data upload, interactive parameter tuning, and PDF download.
- **Consistency**: The web app uses the same functions as the R package, ensuring results are reproducible across platforms.
